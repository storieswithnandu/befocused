...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    try {
        ...
        switch (method) {
            case 'GET': {
                const { rows } = await pool.sql`
                    SELECT * FROM timetable WHERE user_id = ${user.userId} 
                    ORDER BY CASE day
            ...
            case 'POST': {
                ...
                if (!day || !startTime || !endTime || !subject) return res.status(400).json({ message: 'Missing fields' });
                const { rows } = await pool.sql`
            ...
            case 'DELETE': {
                ...
                if (!id) return res.status(400).json({ message: 'ID required' });
                const entryId = parseInt(id.toString());
                const { rowCount } = await pool.sql`DELETE FROM timetable WHERE id = ${entryId} AND user_id = ${user.userId}`;
                if (rowCount === 0) return res.status(404).json({ message: 'Not found' });
        ...
        return res.status(500).json({ message: 'Timetable API error', error: err.message });
    }
}
